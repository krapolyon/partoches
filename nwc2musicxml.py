...
def printAndLog(error_type, file_name, error_string):
    ...
    print(f'Error while {error_type} file {file_name}')
...
def convertFiles(original_files, destination_dir_path):
    ...
    for current_file_path in original_files:
        ...
        path_as_string = str(current_file_path)
        print(f"processing {path_as_string}", end='\r', flush=True)

        try:
            c = parseFile(current_file_path)
        except Exception as e:
            errors += 1
            printAndLog('parsing', path_as_string, str(e))
            continue
        ...
        try:
            ...
            errors += 1
            printAndLog('writing', str(output_path), str(e))
            raise e
            continue
...
def finalize(nb_errors):
    ...
    if errors > 0:
        print(f'terminated with {errors} error(s)')
